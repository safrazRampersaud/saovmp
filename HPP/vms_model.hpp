...
class vms_model
{
	public:
		...
		double cpuReq;
		double memReq;
		double shareable_memReq;
		double non_shareable_memReq;	
		std::string type;
		...
		void requestOperatingSystem(std::vector<vms_model> (&input_V));
		void requestApplications(std::vector<vms_model> (&input_V));
};

vms_model::vms_model() : 	id(0), serverID(-1), active(true), suspend(false), revenue(0.00), 
					cpuReq(0.00), memReq(0.00), shareable_memReq(0.00), non_shareable_memReq(0.00)	
{
	type = {};
	v_PI[saovmp_consts::numOfOps*saovmp_consts::slotSizeOS + saovmp_consts::numOfAps*saovmp_consts::slotSizeAP] = {};
}

vms_model::~vms_model(){}

void vms_model::requestOperatingSystem(std::vector<vms_model> (&input_V))
{
	...
	for(int j = 0; j < input_V.size(); j++)	//For every VM in experiment
	{
		for(int jo = 0; jo < saovmp_consts::slotSizeOS; jo++)	// Initialize the Operating System per VM
			input_V.at(j).OS.push_back(0);
		for(int k = 0; k < vmTypeContainer.size(); k++)	//For every VM Type 
		{		
			if(input_V.at(j).type == vmTypeContainer.at(k))	//If the VM in experiment & if the type is identified, then 
			{
				for(int i = 0; i < saovmp_consts::numOfOPVers; i++)	// For the number of operating systems	
				{
					if(OSconstraintMatrix.at(k).at(i))	// Identify legal OS per VM choice in constraints
					{
						for(int l = 0; l < saovmp_consts::slotSizeOS; l++)
							if(OPS_Pages.at(i).at(l))
								input_V[j].OS[l] = OPS_Pages[i][l];	//Write the bits to VM's OS field.
					}
				}
			}
		}
...
void vms_model::requestApplications(std::vector<vms_model> (&input_V))
{
	int arbPage(0);
	...
	for(int j = 0; j < input_V.size(); j++)	//For every VM in experiment
	{
		...
		{		
			if(input_V.at(j).type == vmTypeContainer.at(k))	//If the VM in experiment & if the type is identified, then 
			{
				...
				for(int i = 0; i < saovmp_consts::numOfAPVers*saovmp_consts::numOfAps; i++)	// For num of apps	
						...
						if((applicationIndices[0] >= 0) && (applicationIndices[0] <= saovmp_consts::numOfAPVers-1))
						{
							for(int l = 0; l < saovmp_consts::slotSizeAP; l++)
								if(APS_Pages.at(applicationIndices[0] % saovmp_consts::numOfAPVers).at(l))
									input_V.at(j).IF.at(l) =  APS_Pages.at(applicationIndices[0] % 
										saovmp_consts::numOfAPVers).at(l);
						}
						else if((applicationIndices[0] >= saovmp_consts::numOfAPVers) && 
								(applicationIndices[0] <= 2*saovmp_consts::numOfAPVers-1))
						{
							for(int l = 0; l < saovmp_consts::slotSizeAP; l++)
								if(APS_Pages.at(applicationIndices[0] % saovmp_consts::numOfAPVers).at(l))
									input_V.at(j).DV.at(l) =  APS_Pages.at(applicationIndices[0] % 
										saovmp_consts::numOfAPVers).at(l);
						}
						else if((applicationIndices[0] >= 2*saovmp_consts::numOfAPVers) && 
								(applicationIndices[0] <= 3*saovmp_consts::numOfAPVers-1))
						{
							for(int l = 0; l < saovmp_consts::slotSizeAP; l++)
								if(APS_Pages.at(applicationIndices[0] % saovmp_consts::numOfAPVers).at(l))
									input_V.at(j).CM.at(l) =  APS_Pages.at(applicationIndices[0] % 
										saovmp_consts::numOfAPVers).at(l);							
						}
						else if((applicationIndices[0] >= 3*saovmp_consts::numOfAPVers) && 
								(applicationIndices[0] <= 4*saovmp_consts::numOfAPVers-1))
						{
							for(int l = 0; l < saovmp_consts::slotSizeAP; l++)
								if(APS_Pages.at(applicationIndices[0] % saovmp_consts::numOfAPVers).at(l))
									input_V.at(j).CR.at(l) =  APS_Pages.at(applicationIndices[0] % 
										saovmp_consts::numOfAPVers).at(l);							
						}
						else if((applicationIndices[0] >= 4*saovmp_consts::numOfAPVers) && 
								(applicationIndices[0] <= 5*saovmp_consts::numOfAPVers-1)) 
						{	
							for(int l = 0; l < saovmp_consts::slotSizeAP; l++)
								if(APS_Pages.at(applicationIndices[0] % saovmp_consts::numOfAPVers).at(l))
									input_V.at(j).DB.at(l) =  APS_Pages.at(applicationIndices[0] % 
										saovmp_consts::numOfAPVers).at(l);							
						}
						else
						{
	...
	{	
		...
		allocation_idx = 0;
		for(int ii = saovmp_consts::slotSizeOS + 1*saovmp_consts::slotSizeAP; 
			ii < saovmp_consts::slotSizeOS + 2*saovmp_consts::slotSizeAP; ii++)
			vm.v_PI[ii] = vm.DV[allocation_idx++];
		allocation_idx = 0;		
		for(int ii = saovmp_consts::slotSizeOS + 2*saovmp_consts::slotSizeAP; 
			ii < saovmp_consts::slotSizeOS + 3*saovmp_consts::slotSizeAP; ii++)
			vm.v_PI[ii] = vm.CM[allocation_idx++];		
		allocation_idx = 0;		
		for(int ii = saovmp_consts::slotSizeOS + 3*saovmp_consts::slotSizeAP; 
			ii < saovmp_consts::slotSizeOS + 4*saovmp_consts::slotSizeAP; ii++)
			vm.v_PI[ii] = vm.CR[allocation_idx++];		
		allocation_idx = 0;		
		for(int ii = saovmp_consts::slotSizeOS + 4*saovmp_consts::slotSizeAP; 
			ii < saovmp_consts::slotSizeOS + 5*saovmp_consts::slotSizeAP; ii++)
