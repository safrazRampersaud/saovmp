	...
	for(int j = 0; j < input_V.size(); j++)	//For every VM in experiment
	{
		...
		{		
			if(input_V.at(j).type == vmTypeContainer.at(k))	//If the VM in experiment & if the type is identified, then 
			{
				...
				{
					if(OSconstraintMatrix.at(k).at(i))	// Identify the legal OS the VM can choose, under the constraint matrix
					{
						for(int l = 0; l < saovmp_consts::slotSizeOS; l++)
							if(OPS_Pages.at(i).at(l))
								input_V[j].OS[l] = OPS_Pages[i][l];	//Write the bits to the VM's OS field.
					}
				}
			}
		}
