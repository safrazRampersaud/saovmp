...
class saovmp_model
{
	public:				
		...
		void initPrototype(std::vector<vms_model> (&input_V), std::vector<srv_model> (&input_S), std::string _vm, std::string _srv);
	...
	private:
		...
		double	originalServerMem[saovmp_consts::SERVER_TYPES];	
...
saovmp_model::saovmp_model() : scarcity_metric(0.00), idx(-1){}
saovmp_model::~saovmp_model() {}

/*
Random-Fit-Sharing: 
One server at a time, assign each VM to the first randomly selected server that fits its request requirements in consideration of page-sharing.
*/
...
{	
	...
	{	
		...
		{	
			...
			{
				for(int pii = 0; pii < saovmp_consts::numOfOps*saovmp_consts::slotSizeOS + saovmp_consts::numOfAps*saovmp_consts::slotSizeAP; pii++)
...
/*
Worst-Fit-Sharing: 
One server at a time, assign each VM to the first server with the most remaining utilizedMemPerServer per scarcity in consideration of page-sharing.
*/
...
/*
Best-Fit-Sharing: 
One server at a time, assign each VM to the server with the least remaining utilizedMemPerServer per scarcity metric following instantiation and considering page-sharing.
*/
...
/*
First-Fit-Sharing: 
One server at a time, assign each VM to the first server that fits its requirements in consideration of page-sharing.
*/
...
void saovmp_model::nextFitSharing()	
{
	...
	{	
		...
		while(allocated == false)
		{				
			if(nfs_srv[activeServer].active == -1)		//	inactive
				nfs_srv[activeServer].active = 1;		//	active
	...
	// Process the allocation results
...
{	
	...
	//	Identify the number of servers which have been opened then closed; thereby having a reduction of resources.
	for(int k = 0; k < process_S.size(); k++)
		if(process_S[k].reduct == true)
			getBinSize++;

	//	Identify the number of active bins
	...
	for(int i = 0; i < saovmp_consts::SERVER_TYPES; i++)
	{
		if(countServerTypes[i] == 0)
			...
			utilizedMemPerServer[i][strat] = countServerTypes[i]*originalServerMem[i] - remainingMemResource[i];
	}
}

void saovmp_model::initPrototype(std::vector<vms_model> (&_V), std::vector<srv_model> (&_S), std::string _vm, std::string _srv)
{
	// Upload VMs for Experiment // 
	std::ifstream open_vm(_vm);
	...
	double _cpuReq = 0.00;
	double _memReq = 0.00;
	double _shareable_memReq = 0.00;
	double _non_shareable_memReq = 0.00;	

	while(open_vm >> _type >> _cpuReq >> _memReq >> _shareable_memReq >> _non_shareable_memReq)
	{
		arbVMS.id 						= ++_id;
		arbVMS.type 					= _type;
		arbVMS.cpuReq 					= _cpuReq;
		arbVMS.memReq 					= _memReq;
		arbVMS.shareable_memReq     	= _shareable_memReq;
		arbVMS.non_shareable_memReq 	= _non_shareable_memReq;
		_V.push_back(arbVMS);
	}
	...
	_id = 0;
	std::ifstream open_srv(_srv);
	if(open_srv.fail()){std::cout << "Error opening: Server Input File.\n"; exit(0);}
	
	// Prep storage variables for uploading Server Configuration; following struct SRV
	srv_model arbSRV = {};
	double _memCap = 0.00;
	double _cpuCap = 0.00;
	int	_countServersPerType = 0;
	int _serverType = 0;
	while(open_srv >> _cpuCap >> _memCap >> _countServersPerType)
	{
		originalServerMem[_serverType] = _memCap; 
		for(int i = 0; i < _countServersPerType; i++)
		{	
			arbSRV.id 			= ++_id;
			arbSRV.type			= _serverType;
			arbSRV.memCap 		= _memCap;
			arbSRV.cpuCap 		= _cpuCap;		
			_S.push_back(arbSRV);
		}
		++_serverType;
	}	
	open_srv.close();
	for(auto &vm : _V)			//Initialize Sharing and Scarcity Fields
	{
		for(auto &s : _S)			
		{
...
void saovmp_model::sharing_per_server(std::vector<vms_model> (&input_V), std::vector<srv_model> (&input_S))
{
	...
	for(int k = 0; k < input_S.size(); k++)
		for(int j = 0; j < input_V.size(); j++)
			for(int i = 0; i < saovmp_consts::numOfOps*saovmp_consts::slotSizeOS + saovmp_consts::numOfAps*saovmp_consts::slotSizeAP; i++)
